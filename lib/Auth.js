...
class Auth {
  ...
  async authorizeRoute(path, jwt, whiteList = []) {
    ...
    if (!jwt) {
      console.log("Auth service cannot find jwt");
      throw new Error("No cookie set");
    }
    const payload = this.verifyJwt(jwt);

    console.log("from the authorizeRoute function in the Auth service; payload = ", payload)
  ...
  verifyJwt(token) {
    ...
    try {
      ...
      console.log("payload from the verifyJwt function", payload);
    } catch (e) {
